...
- `routing.rs` — disk routing: `partition_stage1` and `bucketize_shards` via the shared shard router. Lines with no extractable routing key are silently dropped; the `*_with_key_stats` variants take an optional `AtomicU64` to count those drops (same pattern as the dedupe stage).
