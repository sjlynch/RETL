...
use crate::util::{create_new_with_backoff, remove_with_backoff, replace_file_atomic_backoff};
...
pub fn ensure_staging_dir(out_root: &Path) -> Result<PathBuf> {
    ...
    fs::create_dir_all(&dir).with_context(|| format!("create staging dir {}", dir.display()))?;
    Ok(dir)
}
