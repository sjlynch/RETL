...
- The per-run `run_<token>` shard directory is scratch: `publish.rs` removes it best-effort after a successful merge (logs on failure). A failed merge leaves it for post-mortem. Keep that cleanup so repeated `retl aggregate` runs don't accrete shard JSON under `shards_dir`.
